...
NUM_QUESTIONS_PER_QUIZ = 5  # Caps the n Questions asked regardless of N.
...
def run_quiz():
    ...
    # Postprocess
    print(f"\nYou've got {num_correct} correct! Out of {num} questions.")
...
def ask_question(question):
    ...
    answers = get_answers(
        ...
        hint=question.get("hint"),  # We use this instead of questions['hint'] because not all questions come w/ hints.
    )
    if correct := (set(answers) == set(correct_answers)):
        print("⭐ Correct! ⭐")
    else:
        is_or_are = " is" if len(correct_answers) == 1 else " are"
